...
"""Guided webcam capture of raw images for each sign letter."""
...
def _draw_panel(frame, letter: str, letter_idx: int, total_letters: int,
    ...
    if phase == "ready":
        ...
        msg = f"Completada! {collected} fotos     [ESPACIO] Siguiente     [Q] Salir"
        color = (255, 200, 60)
...
def run_image_capture(config: AppConfig, dataset_root: Path,
                      letters: list[str] | None = None) -> int:
    ...
    try:
        while letter_idx < len(letters_to_capture):
            ...
            while True:
                ...
                if phase == "capture" and collected < _SAMPLES_PER_LETTER:
                    img_path = out_dir / f"{next_idx}.png"
                ...
                _draw_panel(frame, letter, letter_idx, len(letters_to_capture),
                            collected, phase)
                cv2.imshow("Captura de Imagenes LSE", frame)
